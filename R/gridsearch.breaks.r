...
#' @param grid All values to be combnd. As numeric vector. Defaults to seq(0.01, 0.99, 0.01).
                              ...
                              )
{
    ...
    if (!all(!is.na(predictions)) | !all(!is.na(outcomes))) stop("There is missing data in predictions or outcomes")
    ## Loss functions
    loss_functions <- list(auc = function(pred) ROCR::performance(pred, "auc", x.measure =  "cutoff")@y.values[[1]])
    ## Define grid
    breaks_to_search <- combn(grid, n_breaks, simplify = FALSE)
    ...
    if (parallel) {
        ...
        parallel_list <- foreach::foreach(breaks_to_search = parallel_breaks) %dopar% lapply(breaks_to_search, estimate.loss)
