...
#' @export
CreateClassificationTable <- function(predictions.outcome.and.tc) {
    ## Error handling
    ...
    MakeSimpleTable <- function(outcome, priorities,
        ...
        simple.table <- print.xtable(xtable(simple.table,
    ...
    MakeReclassificationTable <- function(superlearner.priorities,
        ...
        reclass.xtable <- xtable(reclass.table,
                                 caption = paste0("\\bf ", caption),
                                 label = label)
        addtorow <- list()
        ...
        reclass.xtable <- print.xtable(reclass.xtable,
                                       add.to.row = addtorow,
                                       include.rownames = FALSE,
        ...
        reclass.xtable <- add.star.caption(reclass.xtable, star.caption)
        return(reclass_xtable)
    }
    ## Settings for classification tables
    settings.classification.tables <- list(superlearner.train.table = list(outcome = predictions.outcome.and.tc$y.train,
    ...
    settings.reclassification.tables <- list(reclass.all = list(superlearner.priorities = predictions.outcome.and.tc$cut.model.test,
    ...
    reclass.tables <- lapply(settings_reclassification_tables, function(setting) {
        with(setting, make.reclassification.table(superlearner.priorities,
                                                  clinicians.priorities,
                                                  caption,
                                                  label))
    })
